function std_joint_x_phase_plot_rot_vel_binned_walk_x_speed(data, steps, flies, param, joint, phase, tossSmallBins, minNumSteps, velocityBins, fig_name, varargin)

%plot standard deviation joint x speed for ctl vs stim binned by forward velocity. 
%
% Optional params:
...
for leg = 1:param.numLegs
    ...
    colors = redblue(numSpeedBins, [min(velocityBins), max(velocityBins)]);
    ...
    for speed = 1:numSpeedBins
        ...
        idxs = find(steps.leg(leg).meta.avg_speed_z > velocityBins(speed) & ...
                    steps.leg(leg).meta.avg_speed_z < velocityBins(speed+1)); %steps with this forward speed bin
        ...
        if plotSpeedData
            ...
            std_joint_x_phase = NaN(1,numPhaseBins);
            numTrials = zeros(numPhaseBins);
    
            for ph = 1:numPhaseBins
                std_joint_x_phase(ph) = std(joint_data(phase_data >= phaseBins(ph) & phase_data < phaseBins(ph+1)));
            end
    
            %plot!
            plt(leg,speed) = plot(phaseBinCenters, smooth(std_joint_x_phase), 'color', colors(speed,:), 'linewidth', 2);hold on
