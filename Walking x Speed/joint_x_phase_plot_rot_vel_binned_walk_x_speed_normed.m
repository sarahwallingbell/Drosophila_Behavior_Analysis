function joint_x_phase_plot_rot_vel_binned_walk_x_speed_normed(data, steps, flies, param, joint, phase, tossSmallBins, minNumSteps, velocityBins, fig_name, varargin)
...
for leg = 1:param.numLegs
    ...
    %calculate average step for this leg (across all speeds) 
    avg_step = NaN(1,numPhaseBins);    
    for ph = 1:numPhaseBins
        idxs = data.([param.legs{leg} phase]) >= phaseBins(ph) & data.([param.legs{leg} phase]) < phaseBins(ph+1); 
        avg_step(ph) = mean(data.([param.legs{leg} joint])(idxs));
    end
    ...
    for speed = 1:numSpeedBins
        ...
        if plotSpeedData
            ...
            %plot!
            plt(leg,speed) = plot(phaseBinCenters, smooth(mean_joint_x_phase-avg_step), 'color', colors(speed,:), 'linewidth', 2);hold on
