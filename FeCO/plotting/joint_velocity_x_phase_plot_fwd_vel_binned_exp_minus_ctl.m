function joint_velocity_x_phase_plot_fwd_vel_binned_exp_minus_ctl(data, steps, flies, param, joint, phase, tossSmallBins, minNumSteps, velocityBins, laserColor, fig_name, varargin)

%plot average joint velocity x speed for ctl vs stim binned by forward velocity. 
%
% Optional params:
...
for leg = 1:param.numLegs
    ...
    for speed = 1:numSpeedBins
        ...
        if plot_ctl_data
           idxs_ctl_ctl = find(steps_ctl.leg(leg).meta.avg_speed_y > velocityBins(speed) & ...
                           steps_ctl.leg(leg).meta.avg_speed_y < velocityBins(speed+1) & ...
            ...
            idxs_exp_ctl = find(steps_ctl.leg(leg).meta.avg_speed_y > velocityBins(speed) & ...
                            steps_ctl.leg(leg).meta.avg_speed_y < velocityBins(speed+1) & ...
                            steps_ctl.leg(leg).meta.percent_stim > 0); %steps with this forward speed bin
        ...
        if plotSpeedData
            %bin data
            joint_data_exp = [diff(data.([param.legs{leg} joint])(vertcat(steps.leg(leg).meta.dataStepIdxs{idxs_exp})))/(1/param.fps); nan];
            phase_data_exp = data.([param.legs{leg} phase])(vertcat(steps.leg(leg).meta.dataStepIdxs{idxs_exp}));
            joint_data_ctl = [diff(data.([param.legs{leg} joint])(vertcat(steps.leg(leg).meta.dataStepIdxs{idxs_ctl})))/(1/param.fps); nan];
            phase_data_ctl = data.([param.legs{leg} phase])(vertcat(steps.leg(leg).meta.dataStepIdxs{idxs_ctl}));
            if plot_ctl_data
                joint_data_exp_ctl = [diff(data_ctl.([param.legs{leg} joint])(vertcat(steps_ctl.leg(leg).meta.dataStepIdxs{idxs_exp_ctl})))/(1/param.fps); nan];
                phase_data_exp_ctl = data_ctl.([param.legs{leg} phase])(vertcat(steps_ctl.leg(leg).meta.dataStepIdxs{idxs_exp_ctl}));
                joint_data_ctl_ctl = [diff(data_ctl.([param.legs{leg} joint])(vertcat(steps_ctl.leg(leg).meta.dataStepIdxs{idxs_ctl_ctl})))/(1/param.fps); nan];
            ...
            %plot!
            ctlplt(speed) = plot(phaseBinCenters, smooth(smooth(mean_joint_x_phase_exp_ctl)-smooth(mean_joint_x_phase_ctl_ctl)), 'color', [Color(param.baseColor) alphas(speed)], 'linewidth', 2); hold on;
            expplt(speed) = plot(phaseBinCenters, smooth(smooth(mean_joint_x_phase_exp)-smooth(mean_joint_x_phase_ctl)), 'color', [Color(laserColor) alphas(speed)], 'linewidth', 2);
