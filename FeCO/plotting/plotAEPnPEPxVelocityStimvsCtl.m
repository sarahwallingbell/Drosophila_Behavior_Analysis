function plotAEPnPEPxVelocityStimvsCtl(steps, param, velocity, type, flies, fig_name, numSpeedBins, minAvgSteps, maxSpeed, onePlot, varargin)

% plot AEP and PEP x velocity - ctl vs stim  
% Sarah Walling-Bell
% November 2022
...
for leg = 1:param.numLegs
    ...
    clear leg_data_A leg_data_B leg_data_A_ctl leg_data_B_ctl
  
    %bin data
    for d = 1:width(directions)
        leg_data_A(:,d) = steps.leg(leg).meta.([metricA '_E_' directions{d}])(stepIdxsExp);
        leg_data_B(:,d) = steps.leg(leg).meta.([metricB '_E_' directions{d}])(stepIdxsExp);
        leg_data_A_ctl(:,d) = steps.leg(leg).meta.([metricA '_E_' directions{d}])(stepIdxsCtl);
        leg_data_B_ctl(:,d) = steps.leg(leg).meta.([metricB '_E_' directions{d}])(stepIdxsCtl);
    end
    ...
    [bins_ctl,binEdges_ctl] = discretize(speed_data_ctl, binEdges);

    %counting flies
    fly_data = steps.leg(leg).meta.fly(stepIdxsExp);
    ...
    mean_leg_data_A_ctl = NaN(numSpeedBins, 3);
    mean_leg_data_B_ctl = NaN(numSpeedBins, 3);
    ...
    for sb = 1:numSpeedBins
        ...
        %ctl
        binned_leg_data_A_ctl = leg_data_A_ctl(bins_ctl == sb, :);
        binned_leg_data_B_ctl = leg_data_B_ctl(bins_ctl == sb, :);
        binned_fly_data_ctl = fly_data_ctl(bins_ctl == sb, :);

        numSteps_ctl(sb) = height(binned_leg_data_A_ctl);
        numFlies_ctl(sb) = height(unique(binned_fly_data_ctl));

        mean_leg_data_A_ctl(sb,:) = mean(binned_leg_data_A_ctl, 'omitnan');
        mean_leg_data_B_ctl(sb,:) = mean(binned_leg_data_B_ctl, 'omitnan');
        numTrials_ctl(sb) = height(binned_leg_data_A_ctl);
    end
    ...
    for sb = 1:numSpeedBins
        ...
        if mean(numTrials_ctl(sb)) < minAvgSteps
            mean_leg_data_A_ctl(sb,:) = NaN; %'erase' these values so they aren't plotted
            mean_leg_data_B_ctl(sb,:) = NaN; %'erase' these values so they aren't plotted
        end 
    end
    ...
    if contains(type, '3D')
        scatter3(mean_leg_data_A(:,1), mean_leg_data_A(:,2), mean_leg_data_A(:,3), dotSize, 1:numSpeedBins, "o", "filled"); hold on; 
        scatter3(mean_leg_data_B(:,1), mean_leg_data_B(:,2), mean_leg_data_B(:,3), dotSize, 1:numSpeedBins, 'square', 'filled'); hold on
        scatter3(mean_leg_data_A_ctl(:,1), mean_leg_data_A_ctl(:,2), mean_leg_data_A_ctl(:,3), dotSize, 1:numSpeedBins, "o"); hold on; 
        scatter3(mean_leg_data_B_ctl(:,1), mean_leg_data_B_ctl(:,2), mean_leg_data_B_ctl(:,3), dotSize, 1:numSpeedBins, 'square'); hold on

    elseif contains(type, '2D')
        scatter(mean_leg_data_A(:,2), mean_leg_data_A(:,1), dotSize, 1:numSpeedBins, "o", 'filled'); hold on; 
        scatter(mean_leg_data_B(:,2), mean_leg_data_B(:,1), dotSize, 1:numSpeedBins, 'diamond', 'filled'); hold on
        scatter(mean_leg_data_A_ctl(:,2), mean_leg_data_A_ctl(:,1), dotSize, 1:numSpeedBins, "o"); hold on; 
        scatter(mean_leg_data_B_ctl(:,2), mean_leg_data_B_ctl(:,1), dotSize, 1:numSpeedBins, 'diamond'); hold on
