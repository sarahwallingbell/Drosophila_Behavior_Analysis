function figHandle = formatFigPolar(figHandle, BandWoption, sbplots, subplotInd)
...
% polar --> indicates plot is polar
% e.g. subplotInd(1).idx = [1,3,4];
% 
% Original by: ES Dickinson, University of Washington, 2020
% Addapted by: S WallingBell, University of Washington, 2021
...
if nargin >= 3 
    ...
    nplots = nrow * ncol; %length(findobj(figHandle,'type','axes'));
    ...
    for n = 1:nplots
        ...
        ax.FontSize = 14; % data by fly
%         ax.FontSize = 30; % all data
        ax.GridColorMode = 'manual'; %makes grid colors GridColor not RColor and ThetaColor
        
        
%%%%%%%%%%%%%%% undo these if possible %%%%%%%%%%%%%%%%%%%%%
        ax.RColor = labelColor; %Color for R labels and ticks
        ax.ThetaColor = labelColor; %Color for Theta labels and ticks
...
%         set(ax, 'color', backColor, 'YColor', labelColor, 'XColor', labelColor)
        ...
        set(labelHandles,'FontSize', 18, 'color', labelColor) % data by fly
%         set(labelHandles,'FontSize', 20, 'color', labelColor) % all data
    end    
else
   % get plot handles
    ax = gca(figHandle);
    ...
    % set the label sizes and color
    labelHandles = findall(ax, 'type', 'text', 'handlevisibility', 'off');
    set(labelHandles,'FontSize', 18, 'color', labelColor) %10 40
end
