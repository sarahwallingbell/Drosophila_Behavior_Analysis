function figHandle = formatFig3D(figHandle, BandWoption, sbplots, subplotInd)
...
% 
% Sarah Walling-Bell, University of Washington, 2022
% Adapted from: ES Dickinson, University of Washington, 2020
...
if nargin >= 3 
    ...
    for n = 1:nplots
        ...
        set(ax, 'color', backColor, 'YColor', labelColor, 'XColor', labelColor, 'ZColor', labelColor)
        % set the font: 
        set(ax, 'FontName', 'Arial');

        % set the label sizes and color
        labelHandles = findall(ax, 'type', 'text', 'handlevisibility', 'off');
        set(labelHandles,'FontSize', 10, 'color', labelColor) %40
    end    
else
   % get plot handles
    ax = gca(figHandle);
