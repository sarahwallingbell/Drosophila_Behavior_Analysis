...
%     '-speed' - if slow_plot == true, this is how much to wait between each frame plotted, in seconds. Default is 0.001 seconds.
%     '-azimuth' -  camera angle rotation around z axis. Default = -90.
%     '-elevation' - camera angle elevation from z = 0. Default = 10;
%     '-background' - background color of plot, true = black, false = white. Default is true, aka black.
%     '-zoom' - zoom level for camzoom(). Greater than 1 is zoom in, less is zoom out. Default is 1.
%
% Example usage:
...
%     
            ...
            if varargin{ii}(1) == '-' %find command descriptions
                switch lower(varargin{ii}(2:end))
                    ...
                    case 'speed'
                        speed = varargin{ii+1};
                    case 'azimuth'
                        azimuth = varargin{ii+1};
                    case 'elevation'
                        elevation = varargin{ii+1};
                    case 'background'
 ...
 % leg_colors = {'blue', 'orange', 'blue', 'orange', 'blue', 'orange'}; %each TRIPOD has its own color
   ...
   if color_stim  %find indices (if any) that are during a stim 
        ...
        elseif param.allLasers(cond) == 0
           ...
           if height(unique(diff(frames))) > 1
               ...
               stim_region(frames >= param.laser_on & frames < param.laser_on+(param.allLasers(cond) * param.fps)) = 1;
   ...
   if color_time %get colors for plotting over time 
    ...
    view(azimuth,elevation) %set camera view
    ax = gca;               % get the current axis
    ax.Clipping = 'off';
    old_camtarget = camtarget;
    camtarget([old_camtarget(1), old_camtarget(2), old_camtarget(3)+Radius]); %center camera on top of ball
    camzoom(zoom); %zoom in on trajectories
