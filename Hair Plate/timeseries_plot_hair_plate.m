function timeseries_plot_hair_plate(data, param, joints, normalize, sameAxes, laserColor, plotSEM, fig_name, varargin)
...
%check to makse sure there's more than one vid for each group of data,
%otherwise don't plot fig (keeps code from crashing) 
enough_vids = true; 
% if width(starts) < 2
%     enough_vids = false; 
% end
% if any(contains(vararginList.VarName, 'data_ctl_in'))
%     if width(starts_ctl_in) < 2
%         enough_vids = false; 
%     end
% end
% if any(contains(vararginList.VarName, 'data_ctl_out'))
%     if width(starts_ctl_out) < 2
%         enough_vids = false; 
%     end
% end
...
if enough_vids
    ...
    % create matrices that hold num flies in plots. 
    ...
    for joint = 1:width(joints)
        ...
        %normalize
        if normalize
            if width(dataMatrix) > 1 %added to catch cases where there's only one vid
        ...
        title([param.legs{leg} ' ' strrep(joints{i}, '_', ' ')]);
    ...
    if sameAxes
        ...
        %plot lasers
        for p = 1:i  
            subplot(width(joints), 1, p); hold on
    ...
    fig = formatFig(fig, true, [width(joints), 1]);
    
    % %label num flies and trials 
    % for l = 1:param.numLegs
    %     subplot(width(joints), 1, nDataIdxs(l)); hold on
